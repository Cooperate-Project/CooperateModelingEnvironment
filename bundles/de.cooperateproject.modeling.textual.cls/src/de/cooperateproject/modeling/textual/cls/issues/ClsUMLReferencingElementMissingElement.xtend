...
class ClsUMLReferencingElementMissingElement extends AutomatedIssueResolutionBase<UMLReferencingElement<Element>> {
	...
	private def dispatch resolvePossible(TypedConnector element) {
		return element.owningPackage.hasReferencedElement && element.left.hasReferencedElement && element.right.hasReferencedElement
	}
	
	private def dispatch resolvePossible(Implementation element) {
		return
			element.owningPackage.hasReferencedElement &&
	...
	private def dispatch resolvePossible(Comment element) {
		if (element.commentedElement instanceof UMLReferencingElement) {
			return (element.commentedElement as UMLReferencingElement).hasReferencedElement
		} else if (element.commentedElement instanceof CommentLink) {
			return (element.commentedElement as CommentLink).commentedElement.hasReferencedElement
		}
		return false
	}
	...
	private def dispatch resolvePossible(Method element) {
		return element.owner.hasReferencedElementOfType(OperationOwner) && !element.parameters.map[type].contains(null) && !element.parameters.map[name].contains(null)
	}
	
	private def dispatch resolvePossible(Parameter element) {
		return
			element.owner.hasReferencedElement &&
			element.type != null &&
			StringUtils.isNotBlank(element.name) &&
	...
	private def dispatch fixMissingUMLElement(Package element) {
		...
		val umlPackage = umlParent.createPackagedElement(element.name, UMLPackage.Literals.PACKAGE) as org.eclipse.uml2.uml.Package
...
//		element.referencedElement = umlAssociation
	...
	private def dispatch fixMissingUMLElement(Implementation element) {
		if (!element.resolvePossible) return Void
		val umlInterfaceRealization = (element.left.referencedElement as org.eclipse.uml2.uml.Class).createInterfaceRealization(null, element.right as org.eclipse.uml2.uml.Interface)
