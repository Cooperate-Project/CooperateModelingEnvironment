...
class SequenceUniqueNameGenerator {
    ...
    public static def String ensureUniqueIdentification(AliasedElement element) {
        if (element.hasLocallyUniqueName && element.name.nameIsValidId) {
            element.name
        } else if (element.isUniqueAlias(element.alias)) element.alias 
        else element.generateUniqueAlias
    }
    ...
    private static def boolean isUniqueAlias(AliasedElement element, String uniqueAlias) {
        (!element.alias.nullOrEmpty) && EcoreUtil.getRootContainer(element)
           .eAllContents.filter(AliasedElement).filter[alias == uniqueAlias].forall[it == element]
    }
    ...
    private static def generateUniqueAlias(AliasedElement element) {
        ...
        var counter = 0
        var newAlias = name
        while (!element.isUniqueAlias(newAlias)) {
            newAlias = '''«name»«counter++»'''
        }
        newAlias
    }
