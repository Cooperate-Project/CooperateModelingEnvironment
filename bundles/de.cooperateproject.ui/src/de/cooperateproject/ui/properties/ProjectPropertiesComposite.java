...
public class ProjectPropertiesComposite extends Composite {
	 ...
	 * @param style
	 */
	public ProjectPropertiesComposite(Composite parent, int style, ProjectPropertiesDTO preferencesDTO, IChangeListener validatorStatusListener) {
		super(parent, style);
	...
	protected DataBindingContext createDataBindings() {
		...
		// atomic host
		IObservableValue<String> observeTextTxtCDOHostnameObserveWidget = WidgetProperties.text(SWT.Modify).observe(txtCDOHostname);
		...
		bindingContext.bindValue(observeTextTxtCDOHostnameObserveWidget, atomicValidatedHostname, strategyAtomicStringToModel, strategyAtomicStringToTarget);
		// atomic port
		IObservableValue<String> observeTextTxtCDOPortObserveWidget = WidgetProperties.text(SWT.Modify).observe(txtCDOPort);
		...
		// atomic repo
		IObservableValue<String> observeTextTxtCDORepositoryObserveWidget = WidgetProperties.text(SWT.Modify).observe(txtCDORepository);
		IObservableValue<String> atomicValidatedRepository = new WritableValue<String>(null, String.class);
		bindingContext.bindValue(observeTextTxtCDORepositoryObserveWidget, atomicValidatedRepository, strategyAtomicStringToModel, strategyAtomicStringToTarget);
		// connection validation
		MultiValidator connectionValidator = new CDOCredentialsValidator(atomicValidatedHostname, atomicValidatedPort, atomicValidatedRepository);
		...
		// model bindings
		IObservableValue<String> cdoHostPreferencesDTOObserveValue = BeanProperties.value("cdoHost").observe(preferencesDTO);
		bindingContext.bindValue(validatedHostname, cdoHostPreferencesDTOObserveValue);
		IObservableValue<Integer> cdoPortPreferencesDTOObserveValue = BeanProperties.value("cdoPort").observe(preferencesDTO);
		bindingContext.bindValue(validatedPort, cdoPortPreferencesDTOObserveValue);
		IObservableValue<String> cdoRepoPreferencesDTOObserveValue = BeanProperties.value("cdoRepo").observe(preferencesDTO);
