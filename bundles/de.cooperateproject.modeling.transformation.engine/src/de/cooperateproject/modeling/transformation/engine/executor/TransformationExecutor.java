...
public enum TransformationExecutor {
	...
	public void transformChanged(URI changedResource, ResourceSet resourceSet) throws IOException {
		Collection<TransformationFactory> transformationFactories = TransformationFactoryRegistry.getInstance().findTransformationFactory(changedResource, resourceSet);	
		Collection<Transformation> transformations = transformationFactories.stream().map(f -> f.create(changedResource, resourceSet)).collect(Collectors.toSet());
